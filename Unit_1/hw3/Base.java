...
public class Base implements Factor, Comparable<Base> {
    ...
    public Result getDerivative(Type type) {
        final Result result = new Result(false);
        int index = type.ordinal();
        Base b = this.cloneSimple();
        if (this.exp[index].compareTo(BigInteger.ZERO) > 0) {
            b.coff = b.coff.multiply(this.exp[index]);
            b.exp[index] = this.exp[index].add(BigInteger.ONE.negate());
            b.sins.addAll(this.sins);
            b.coss.addAll(this.coss);
            result.addBase(b);
        }
        for (int i = 0; i < sins.size(); i++) {
            ...
            for (int j = 0; j < sins.size(); j++) {
                if (i != j) {
                    b.sins.add(sins.get(j));
                }
            }
            Result newRes = new Result(false);
            newRes.addBase(b);
            newRes.multiResult(sins.get(i).getDerivative(type));
            result.addResult(newRes);
        }
        for (int i = 0; i < coss.size(); i++) {
            b = this.cloneSimple();
            b.sins.addAll(this.sins);
            for (int j = 0; j < coss.size(); j++) {
                if (i != j) {
                    b.coss.add(coss.get(j));
                }
            }
            Result newRes = new Result(false);
            newRes.addBase(b);
            newRes.multiResult(coss.get(i).getDerivative(type));
            result.addResult(newRes);
        }
        return result;
    }

    public Base cloneSimple() {
        Base res = new Base(this.coff);
        res.exp[0] = this.exp[0];
        res.exp[1] = this.exp[1];
        res.exp[2] = this.exp[2];
        return res;
    }
